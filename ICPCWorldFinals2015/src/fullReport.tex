...
be modeled reasonably accurately with the following equation:
...
be looking at each subsequent highest corresponding following lowest points $\implies$ we will
...
\begin{proof}
This is the case because our algorithm goes throughh the points $1, 2, ..., n$ once and only
...
Then starting with the second point until the 10th we will read throughh and record the values of what h, l, curDiff,
and diff are:
...
%%%%%%% Mathematical Formulation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mathematical Formulation}
Given an input of $k$ teams and $n$ jobs that must be fulfilled, we are given the order that the jobs occur in
...
\begin{algorithmic}
    \Procedure{Control}{}
        \For{each test case}
            \State \Call{buildGraph}{ }
            \State print(\Call{match}{ })
        \EndFor
    \EndProcedure
    \Procedure{buildGraph}{}
        ...
        \For{each origin node $n, n\in [iFrom,iTo-1]$}
            \State connect(s, n, 0)
        \EndFor
        \For{each origin node $n, n\in [iFrom,iTo-1]$}
            \For{each destination node $m, m\in [iTo,t-1]$}
                \State connect(n, m, costFromInput)
            \EndFor
        \EndFor
        \For{each destination node $n, n\in [iFrom,iTo-1]$}
            \State connect(n, t, 0)
        \EndFor
    \EndProcedure
\end{algorithmic}
...
true bipartite graph this is discussed in the correctness section). To preform this we make a copy of our
graph to which we will be altering while referencing our original (very important) then we will define a
cost array to store the cost of each node. From here we find our shortest path using dijkstra's algorithm
...
\begin{algorithm}[H]
\caption{Min Cost Bipartite Matching}
\begin{algorithmic}
    \Procedure{match}{}
        ...
        \State int $minimum = 0$
        \For{each origin node $n, n\in [iFrom,iTo-1]$}
            \For{each node $m, m\in [1,numNodes]$}
                \If{graph[n][m] != Infinity \&\& defCopyG[n][m] != Infinity}
...
%%%%%%% PROPOSITION 1 %%%%%%%%%%%%%%%
\begin{proposition}
~ \\ \indent Solving min-cost bipartite matching on our graph construction will give us the correct answer.
\end{proposition}
...
\begin{proof}
~ \\ \indent This is due to the fact that all of our information is stored in either one dimensional or
two dimensional integer arrays or a minimum priority queue, each of which has the size comparable to
our number of nodes. The largest of these therefore is our two dimensional array of size $(numNodes)^2$.
...
take V+E time which is O($N^2$). The next part is the performance of our min-cost bipartite algorithm which
will go trough for number of jobs (n) + and perform dijkstra's $(E + V\cdot log(V)) \implies (N^2) + N\cdot log(N)$
, O($N^2$) then we will update costs which each time (4) will at most go throughh the entire graph once O($N^2$)
...
\indent The only data structure we used was a two dimensional array \textbf{holes$[n][2]$} such that
each entry $holes[i][0]$ corresponds to the center $z$ coordinate and $holes[i][1]$ is the radius of hole $i$.
Aside from this we use the instance variable $v, goal, numHoles, numSlices$ to keep track of the total volume,
...
\subsection{Correctness}
\begin{corollary}
~ \\ \indent It is sufficient for us to simply view the spheres on a 1-dimensional plane because we are
...
calculate the volume displaced using only the portion $(i_a, i_b)$ that overlaps with sphere $i$ and its
radius, $i_r$.
\end{corollary}
%%%%%%% PROPOSITION 1 %%%%%%%%%%%%%%%
\begin{proposition}
...
can calculate or lookup each intermediate stage. Therefore if we perform $s$ binary searches, decreasing
our range appropriately each time we find a cut, and we know our stopping criteria as $goal$; we can
calculate the volume displaced by each portion of a sphere in intermediate ranges $[a,b]$, ultimately
...
we do at most (worst-case if every interval contains every hole) $N$ volume calculations.
\begin{center}
...
\begin{algorithmic}
    ...
    \Procedure{initialize}{ }
        ...
        \State \Call{sort}{sequence} by ascending order
...
and the queue. Worst case the sum of the sizes of the two stacks and the queue will be $N$ because each string
...
With pocket-size devices, the possibilities for typing text are often limited. Some devices provide only
a few buttons, significantly fewer than the number of letters in the alphabet. For such devices, several
...
over an appropriate key, pressing the fifth button selects the corresponding character and appends it
...
only five hardware buttons. In this problem, you are given a virtual keyboard layout and your task
is to determine the minimal number of strokes needed to type a given text, where pressing any of the
five hardware buttons constitutes a stroke. The keys are arranged in a rectangular grid, such that each
...
determine the minimum number of keystrokes that are required to type in the string that is used which consist
...
the corresponding key for that level we move up. \\
...
on the keyboard throughh a bfs originating at $k_i$. Therefore we enact a $S$ depth bfs on the keyboard such that
...
\begin{proof}
~ \\ \indent This is due to the fact that we are storing a three dimensional array or $S$ 2-D boolean array of the
...
of two InvalidIntervals will be compared. These InvalidInterval start and end times are computed utilizing the basic
function $time = \frac{distance}{velocity}$. We treat each east and west bound ship the same simply by multiplying the
east bound boats starting position by $-1$. Once we have built up our set of InvalidIntervals, we sort them and then
perform a greedy algorithm on them to determine the largest valid start interval for the ferry (see correctness for
...
\begin{algorithmic}
    \Procedure{main}{}
        \For{each case}
            ...
            \For{$lane \in [1, numLanes]$}
                ...
                \For{numShips}
                    ...
                    \If{$end\ \textless\ fStart\ ||\ start\ \textgreater\ fEnd$}
                        continue;
                    \EndIf
        ...
        \State \Call{determineMaxValidInterval}{invalidIntervals}
    \EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}
    \Procedure{determineMaxValidInterval}{invalidIntervals}
        ...
        \State $curGap$
        \For{$curShip \in [1..numShips]$}
            \If{invalidIntervals[curShip].getStart() \textgreater endTime}
...
\begin{proof}
~ \\ \indent We can determine the time at which each ship will be crossing the path of the ferry and therefore also the
...
\begin{proof}
~ \\ \indent We do this through a greedy approach which the main idea is to sort all of the intervals in increasing
...
guaranteed to also overlap. We linearly go throughh the list keeping track of our last known position as well as our
largest gap so that way if there is no overlap between the last known position and the start of the next interval,
we are guaranteed that this is a gap, therefore we compare it to the longest gap that we know thus-far and if it is
longer, we will know the longest gap. Thus when we go throughh every interval we are guaranteed to find the largest gap.
\end{proof}
...
list into an array O(N) and sort them by earliest start time O($N\cdot log(N)$). Then we go throughh
each interval for each ship linearly keeping track of our largest gap seen thus-far O(N). Then we
...
that in the first lane, there are 4 boats are traveling West-bound. The $1^{st}$ one is located at
position 100 with a length of 100, the $2^{nd}$ located at position 300 with a length of 100, the
$3^{rd}$ located at position 700 with a length of 100 and the $4^{th}$ located at position 900 with
a length of 100.
\newpage
We note here that none of the ships will be overlapping at any point since they are all in the same
lane, with 100 units between each of them and are all traveling the same speed. We calculate then
...
by saying our largest window is 10.0 - 0 = \textbf{10.0} and keep track of the farthest endpoint seen thus-far,
40.0. Then we check to see if our farthest endpoint and the start of interval 2 overlap, they do not
so we check the gap = 10.0 which is the same so we update our farthest endpoint to be 80.0 and continue.
Following this we check to see if 80.0 \textgreater start of interval 3, it is not so we check the gap
which is 130.0 - 80.0 = \textbf{50.0} which is our largest gap thus-far so we record it as well as the farthest
